...
class UserClocksSheet implements FromCollection, WithHeadings, WithStyles, WithColumnFormatting, WithTitle, WithEvents, WithCustomStartCell
{
    ...
    public function collection()
    {
        ...
        $timezoneOffset = $user->timezone ? $user->timezone->value : 0;
        ...
        for ($date = $startDate->copy(); $date->lte($endDate); $date->addDay()) {
            ...
            $clockInFormatted = $hasMultipleSegments ? '' : ($earliestIn ? $earliestIn->copy()->addHours($timezoneOffset)->format('h:i A') : '');
            $clockOutFormatted = $hasMultipleSegments ? '' : ($latestOut ? $latestOut->copy()->addHours($timezoneOffset)->format('h:i A') : '');
            ...
            if ($hasMultipleSegments) {
                foreach ($dailyClocks as $seg) {
                    $segIn = $seg->clock_in ? Carbon::parse($seg->clock_in, 'UTC')->addHours($timezoneOffset)->format('h:i A') : '';
                    $segOut = $seg->clock_out ? Carbon::parse($seg->clock_out, 'UTC')->addHours($timezoneOffset)->format('h:i A') : '';
                    $final->push([
                        'Date' => '',
                        'Clock In' => $segIn,
                        'Clock Out' => $segOut,
